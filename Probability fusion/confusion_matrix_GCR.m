function [confusion, accuracy, TPR, AA, KA, FPR] = confusion_matrix_GCR(class, c)
%
...
for i = 1: c_len
  ...
  TPR(i) = confusion(i, i)/sum(confusion(i, :)); %CA
end
accuracy = sum(diag(confusion))/sum(c); %OA
AA = mean(TPR);
KA = (sum(confusion(:))*sum(diag(confusion)) - sum(confusion)*sum(confusion,2))...
    /(sum(confusion(:))^2 -  sum(confusion)*sum(confusion,2));
